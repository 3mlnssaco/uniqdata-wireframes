...
Enterprise Portal(Spring Boot)은 Core 서버(FastAPI)와 **Internal API**로 통신.
...
```
Layer 1: wallet_address      → Core core_auth에만 존재 (Enterprise 접근 불가)
Layer 2: person_id (UUID)    → Core uq_cdm에만 존재 (Enterprise 접근 불가)
Layer 3: participant_number  → Enterprise가 보는 유일한 식별자
```

Enterprise는 `participant_number`만 취급. `person_id`, `wallet_address`는 절대 전달되지 않음.

---

## 1. 연구자 인증

Enterprise 자체 인증 (Spring Boot).
...
```
- Core가 앱 푸시 발송, Enterprise는 발송 건수만 수신
...
환자 앱에서 동의 승인
    ↓
[Core → Enterprise 푸시]
POST /api/enterprise/project/{id}/participant
...
## 6. 데이터 수신 (Core → Enterprise 푸시)

### 6.1 데이터 전송
```
[Core → Enterprise]
POST /api/enterprise/project/{id}/dataset
Headers: X-Service-Auth: {token}
Body: {
    participant_number: "P001",
    records: [
...
```
[Core → Enterprise]
DELETE /api/enterprise/project/{id}/participant/{participant_number}
Headers: X-Service-Auth: {token}
```
- Enterprise는 해당 참여자 데이터 즉시 삭제

### 6.3 회원 탈퇴
```
[Core → Enterprise]
POST /enterprise/push/withdrawal
Headers: X-Service-Auth: {token}
...
| 역할 | Core 데이터 | Enterprise 데이터 | 식별자 |
|------|------------|-------------------|--------|
...
## 11. Core 아웃바운드 (Core → Enterprise 푸시)

| Method | Path | 용도 |
|--------|------|------|
| POST | `/api/enterprise/project/{id}/participant` | 참여자 등록 알림 |
| POST | `/api/enterprise/project/{id}/dataset` | 데이터 전송 |
| DELETE | `/api/enterprise/project/{id}/participant/{number}` | 동의 철회 |
| POST | `/enterprise/push/withdrawal` | 회원 탈퇴 |
